...
function importarJulho(csvPath) {
            ...
            db.get('SELECT id FROM pessoas WHERE nome = ?', [nome], (err, pessoa) => {
                ...
                if (pessoa) {
                    db.run(
                        ...
                        [pessoa.id, 'comum', valorJulho, '2025-07-06', new Date().toISOString()],
                        function(err) {
                            if (err) {
                                ...
                                importados++;
                                console.log(`✅ ${nome} - Julho: ${valorJulho}`);
                            }
                        }
                    );
                } else {
...
if (!csvFile) {
    console.log('❌ Uso: node importar_julho.js <arquivo.csv>');
    process.exit(1);
}

importarJulho(csvFile);