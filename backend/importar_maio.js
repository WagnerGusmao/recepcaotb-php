            ...
            db.get('SELECT id FROM pessoas WHERE nome = ?', [nome], (err, pessoa) => {
                ...
                if (pessoa) {
                    db.run(
                        ...
                        [pessoa.id, 'comum', valorMaio, '2025-05-04', new Date().toISOString()],
                        function(err) {
                            if (err) {
                                ...
                                importados++;
                                console.log(`✅ ${nome} - Maio: ${valorMaio}`);
                            }
                        }
                    );
                } else {
...
if (!csvFile) {
    console.log('❌ Uso: node importar_maio.js <arquivo.csv>');
    process.exit(1);
}

importarMaio(csvFile);